const STATIC_CACHE_NAME = 'static-cache-v1.1';//guarda app shel (todo lo necesario para que nuestra funcione bien)(incluyendo la ruta)
...
const respCache = caches.open(STATIC_CACHE_NAME).then((cache) => {
  return cache.addAll([
      ...
      '/css/style.css',
      '/js/app.js',
      '/manifest.json'//aqui se agrega el manifest (estamos dentro de la funcion "Install")
  ])
})
...
//rutas de los recusrsos externos(apis)
...
//esperamos a que los caches se guarden correctamente para instalarse
...
//comparar los cache y lo limpia
